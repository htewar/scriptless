import {Plus, List, History, LayoutGrid, Settings2} from 'lucide-react';
...
const SideBarOptions = (platform: string) => [
    ...
    },
    {
        name: "Localization",
        icon: <Settings2 />,
        route: RoutePaths.Utilities(platform)
    },
];
            ...
            <div className='flex flex-col gap-4 mt-8'>
                {
                    SideBarOptions(platform).map(
                        (option, index) => (
                            <Button
                                key={index}
                                className='flex gap-2 items-center justify-start border border-black
                                hover:bg-black/50 hover:text-white hover:border-transparent'
                                variant="outline"
                                onClick={() => route.replace(option.route)}
                            >
                                {option.icon}
                                {option.name}
                            </Button>
